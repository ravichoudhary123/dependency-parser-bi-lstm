...
class BiLSTMWordEmbeddingLookup(nn.Module):
    ...
    def forward(self, sentence):
        ...
        wordids = map(lambda w: self.word_to_ix[w], sentence)
        tensor = torch.LongTensor(wordids)
        inputs = ag.Variable(tensor)
        wordembeds = self.word_embeddings(inputs)
        
        lstm_output, self.hidden = self.lstm(wordembeds.view(len(sentence),1,-1),self.hidden)
        return [i for i in lstm_output]
...
class LSTMCombinerNetwork(nn.Module):
    ...
    def forward(self, head_embed, modifier_embed):
        ...
        # STUDENT
        wordembeds = utils.concat_and_flatten([head_embed,modifier_embed])
        lstm_output, self.hidden = self.lstm(wordembeds.view(1,1,-1),self.hidden)
